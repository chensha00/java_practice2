...
public class GoodsDaoImpl implements GoodsDao {
     ...
     */
    @Override
    public int updateGoods(Goods goods, Connection connection, PreparedStatement preparedStatement) throws SQLException {
        ...
        int operatedRow=preparedStatement.executeUpdate();
        connection.commit();
        return operatedRow;
    }
    /**
    ...
    public Goods insertGoods(Goods Goods, Connection connection, PreparedStatement preparedStatement) throws SQLException {
        ...
        preparedStatement.setDouble((int)4, goods.getPrice());
        preparedStatement.setString((int)5, goods.getAddr());
        java.sql.Date sqlDate=new java.sql.Date(goods.getMakeTime().getTime());
        preparedStatement.setDate((int)6,sqlDate);
          sqlDate=new java.sql.Date(goods.getExpirationTime().getTime());
    ...
    public Goods queryGoodsById(Long id, Connection connection, PreparedStatement preparedStatement) throws SQLException {
        preparedStatement=connection.prepareStatement("SELECT NAME,TYPE,NUMBER ,PRICE FROM goods WHERE ID=?;");
        preparedStatement.setLong(1,id);
        //获取查询的结果集
    ...
    public List<Goods> queryGoodsByCondition(Map<String, String> stringGoodsMap, Connection connection, PreparedStatement preparedStatement) throws SQLException {
        ...
        for(String key:keySet){
            ...
            preparedStatement=connection.prepareStatement("SELECT NAME,TYPE,NULLABLE,PRICE FROM goods WHERE ?=?;");
